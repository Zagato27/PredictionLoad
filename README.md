...
- Основной latency-прогноз: эмпирическая кривая observed avg/max latency vs total RPS.
- Диагностические модели очередей: M/M/1, M/M/c (Erlang C), Kingman (G/G/1), G/G/c (Allen–Cunneen).
...
- Рекомендации по масштабированию: инстансы по CPU/RAM; `c` используется в диагностических queueing-моделях.
...
- `services/forecast.py` — логика валидации, empirical latency forecast, ресурсных регрессий и диагностических queueing-моделей.
...
1) Таблица «Основной latency-прогноз»  
- Основной прогноз строится по observed avg/max latency vs total RPS.  
- Queueing-модели M/M/1, M/M/c, Kingman и G/G/c показываются отдельно как инженерная диагностика.

2) Таблица «Параметры и оценки»  
- `S (мс)`: сервисное время для queueing-диагностики — минимальный `avg_ms` на линейном участке (low-load).  
- `D_cpu`, `D_ram`: наклон регрессии `util ~ rps_per_pod` (с intercept при значимом базовом расходе).  
- `Ca`, `Cs`: диагностические прокси (разброс RPS между ступенями и tail ratio `(max-avg)/avg`), не классические CV.  
- `k_peak`: максимальное отношение `max_ms/avg_ms` по ступеням для queueing-max.  
- Requests/Limits per pod: агрегированные средние значения в ресурсной таблице.

3) График «Задержка vs RPS»  
- Точки Observed — фактические средние и максимальные задержки на ступенях теста.  
- Линии Empirical avg/max — основной прогноз по наблюдаемой кривой.  
- Линии M/M/1, M/M/c, G/G/1, G/G/c — диагностические queueing-модели.  
...
- Линии CPU и RAM — **per-pod util** при автоматическом числе реплик `ceil(demand / u_max)` на каждом RPS; сравниваются с `u_max_cpu`/`u_max_ram`.  
- Tooltip также показывает `demand` (эквивалент нагрузки на один pod без масштабирования).  
- В таблице: `Demand до масштабирования` = `intercept + slope * target_rps`; `Util после N реплик` = demand / N.

5) График «Инстансы vs RPS»  
- Ступенчатые линии: инстансы по CPU, RAM и IO; `suggested_m = max(cpu, ram, io)`.
...
```
`u_max_cpu` и `u_max_ram` задаются как доли от requests на pod (`0.8` = 80%). `mmc_c_optional` — число каналов для M/M/c; если не задано, оценивается как `mean(pods × concurrency_optional)` или `mean(pods)`. Queueing-диагностика: M/M/c и G/G/c с `λ=total RPS` и `c=max(реплики, mmc_c)`; M/M/1 и Kingman с `λ=total RPS / c`.
