...
- Линии CPU и RAM — прогноз per-pod утилизации; `rps` в ступени остаётся total RPS, а demand фитится через `total_rps / pods`.  
- Горизонтальные пороги `u_max_cpu`/`u_max_ram` показывают границы, где потребуется масштабирование.
- В отчёте `Demand до масштабирования` показывает эквивалентную нагрузку на один pod при всём target RPS; `Util после N реплик` показывает ожидаемую per-pod утилизацию после рекомендованного масштабирования.
...
```
`u_max_cpu` и `u_max_ram` задаются как доли от requests на pod (`0.8` = 80%). `mmc_c_optional` — число параллельных обработчиков для диагностических queueing-моделей, примерно `pods × workers_per_pod`.
